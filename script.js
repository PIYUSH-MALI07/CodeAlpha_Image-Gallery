...
const downloadUhdBtn = document.getElementById('downloadUhdBtn');
...
const downloadHdBtnMobile = document.getElementById('downloadHdBtnMobile');
const downloadUhdBtnMobile = document.getElementById('downloadUhdBtnMobile');
const lightboxCategoryMobile = document.getElementById('lightboxCategoryMobile');
const lightboxCaptionMobile = document.getElementById('lightboxCaptionMobile');
...
// Dynamic Gallery Render Function (Staggered Cascade Animation)
function renderGallery() {
    ...
    imagesData.forEach((item) => {
        ...
        if (matchesCategory && matchesSearch) {
            ...
            card.classList.add('gallery-card', 'ripple-container');

            const delay = count * 0.06;
            card.style.animation = `cardCascadeIn 0.55s cubic-bezier(0.16, 1, 0.3, 1) ${delay}s forwards`;
...
// Back to Top Button — show on scroll, smooth scroll on click
...
function openLightbox(index) {
    ...
    lightboxImg.src = buildSrc(currentItem.src, 1920);
    ...
    if (lightboxCategoryMobile) lightboxCategoryMobile.textContent = currentItem.category.toUpperCase();
    if (lightboxCaptionMobile) lightboxCaptionMobile.textContent = currentItem.title;
...
// Build a higher-resolution source URL from a base image URL
function buildSrc(src, width) {
    return src.replace(/w=\d+/, `w=${width}`);
}

// Direct Image Download Trigger Function (HD = 1920px, UHD = 3840px)
function downloadCurrentImage(width) {
    if (!currentVisibleItems[currentIndex]) return;
    
    const currentItem = currentVisibleItems[currentIndex];
    const imageSrc = buildSrc(currentItem.src, width);
    const quality = width >= 3840 ? 'UHD' : 'HD';
    const fileName = `${currentItem.title.toLowerCase().replace(/\s+/g, '-')}-${quality}.jpg`;
...
downloadHdBtn.addEventListener('click', () => downloadCurrentImage(1920));
downloadUhdBtn.addEventListener('click', () => downloadCurrentImage(3840));

// Mobile toolbar event listeners
if (backBtnMobile) backBtnMobile.addEventListener('click', closeLightbox);
if (closeBtnMobile) closeBtnMobile.addEventListener('click', closeLightbox);
if (downloadHdBtnMobile) downloadHdBtnMobile.addEventListener('click', () => downloadCurrentImage(1920));
if (downloadUhdBtnMobile) downloadUhdBtnMobile.addEventListener('click', () => downloadCurrentImage(3840));
...
tabBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        // 4 columns for All, 3 columns for specific categories
        if (currentCategory === 'all') {
            galleryGrid.classList.remove('three-col');
        } else {
            galleryGrid.classList.add('three-col');
        }
...
let loggedUser = JSON.parse(localStorage.getItem('galleryUser')) || null;

// Navbar button update based on login state
function updateAuthButton() {
    if (loggedUser) {
        loginOpenBtn.style.display = 'none';
        signinOpenBtn.innerHTML = `👤 ${loggedUser.name.split(' ')[0]}`;
        signinOpenBtn.classList.add('auth-logged-in', 'neon-pulse-green');
        signinOpenBtn.classList.remove('neon-pulse-btn');
    } else {
        loginOpenBtn.style.display = 'flex';
        signinOpenBtn.innerHTML = '✨ Sign In';
        signinOpenBtn.classList.remove('auth-logged-in', 'neon-pulse-green');
        signinOpenBtn.classList.add('neon-pulse-btn');
        loginOpenBtn.classList.add('neon-pulse-btn');
    }
}
...
document.addEventListener('click', (e) => {
    const btn = e.target.closest('.auth-btn, .theme-toggle-btn, .tab-btn, .back-btn, .download-btn, .fullscreen-exit-btn, .auth-submit-btn');
...
function openAuthModal(formType) {
    if (loggedUser) {
        if (confirm(`Logged in as ${loggedUser.email}\n\nDo you want to logout?`)) {
            loggedUser = null;
...
signinForm.addEventListener('submit', (e) => {
    ...
    const users = JSON.parse(localStorage.getItem('galleryUsers')) || [];
    ...
    signinForm.reset();
    authOverlay.classList.remove('active');
    showToast(`Welcome ${name}! Account created & logged in.`);
});
...
// Populate UHD photo count stat
const statNumberEl = document.querySelector('.stat-number');
if (statNumberEl && statNumberEl.textContent.trim() === '') {
    statNumberEl.textContent = imagesData.length;
}
